...
// cnx a la base
mysqli_select_db($mysqli, DATABASE) or die('Erreur de connection � la BDD : ' . mysqli_connect_error());
// FORCE UTF-8
...
While ($num_page <= $nb_page) {
    ...
    if ($num_page == $nb_page) {
        ...
        // reglement
        $pdf->SetXY(5, 225);
        $pdf->Cell(38, 5, "Mode de R�glement :", 0, 0, 'R');
        $pdf->Cell(55, 5, $row[6], 0, 0, 'L');
        // echeance
        $champ_date = date_create($row[7]);
        $date_ech   = date_format($champ_date, 'd/m/Y');
        $pdf->SetXY(5, 230);
        $pdf->Cell(38, 5, "Date Ech�ance :", 0, 0, 'R');
    ...
    if ($row_client[4] || $row_client[5]) {
        $pdf->SetXY($x, $y);
        $pdf->Cell(100, 8, $row_client[4] . ' ' . $row_client[5], 0, 0, '');
        $y += 4;
    }
    if ($row_client[6]) {
        $pdf->SetXY($x, $y);
        $pdf->Cell(100, 8, 'N� TVA Intra : ' . $row_client[6], 0, 0, '');
    }
    ...
    $pdf->SetXY(1, 96);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(140, 8, "Libell�", 0, 0, 'C');
    $pdf->SetXY(145, 96);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(13, 8, "Qt�", 0, 0, 'C');
    $pdf->SetXY(156, 96);
    $pdf->SetFont('Arial', 'B', 8);
    ...
    while ($data = mysqli_fetch_assoc($res)) {
        // libelle
        $pdf->SetXY(7, $y + 9);
        $pdf->Cell(140, 5, $data['libelle'], 0, 0, 'L');
        // qte
        $pdf->SetXY(145, $y + 9);
        $pdf->Cell(13, 5, strrev(wordwrap(strrev($data['qte']), 3, ' ', true)), 0, 0, 'R');
        // PU
    ...
    if ($num_page == $nb_page) {
        ...
        while ($data = mysqli_fetch_assoc($res)) {
            $pdf->SetXY($x, 221);
            $pdf->Cell(17, 6, $data['taux_tva'] . ' %', 0, 0, 'C');
            $taux = $data['taux_tva'];

            $nombre_format_francais = number_format($data['tot_ht'], 2, ',', ' ');
            $pdf->SetXY($x, 227);
            $pdf->Cell(17, 6, $nombre_format_francais, 0, 0, 'R');
            $col_ht = $data['tot_ht'];
