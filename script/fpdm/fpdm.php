...
$FPDM_FILTERS = []; //holds all supported filters
$FPDM_REGEXPS = [
    "/Type"    => "/\/Type\s+\/(\w+)$/",
    "/Subtype" => "/^\/Subtype\s+\/(\w+)$/",
];
...
$__tmp = version_compare(phpversion(), "5") == -1 ? ['FPDM'] : ['FPDM', false];
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    class FPDM {
        ...
        function FPDM() {
            ...
            switch ($num_args) {
                ...
                case FPDM_COMMON: //Common use
                    ...
                    $this->security = ['password' => ['owner' => null, 'user' => null], 'encrypt' => 0, 'allow' => []];
        ...
        function Fix() {
            //---------------
            if (!$this->fields) $this->fields = []; //Default: No field data
...
//							$this->set_field_value("tooltip",$name,$value);
        ...
        function Output($dest = '', $name = '') {
            ...
            if ($this->support == "pdftk") {
                ...
                $ret = pdftk($pdf_file, $fdf_file, ["security" => $security, "output_modes" => $output_modes]);
        ...
        function static_method_call($method) {
            ...
            return call_user_func_array([$this, $method], $params_call);
        }
...
//									if(!array_key_exists("default",$values)) $this->Error("Cannot find default value (/DV) for field $name");
                                ...
                                if (preg_match("/\/Length\s*(\d+)/", $CurLine, $match)) {
                                    $stream["length"]  = ["line" => $Counter, "value" => $match[1]];
                                    $stream["start"]   = 0;
                                    $stream["end"]     = 0;
                                    $stream["content"] = '';
                                ...
                                if (preg_match_all($this->streams_filter, $CurLine, $matches)) {
                                    ...
                                    $stream["filters"] = ["line" => $Counter, "type" => $stream_filters];
                                    if ($verbose_parsing) {
...
//									}
                        ...
                        if (preg_match("/\bxref\b/", $CurLine, $match)) {
                            ...
                            $lines['$_XREF_$']                              = [];
                            $lines['$_XREF_$']["entries"]                   = [];
                            $lines['$_XREF_$']["infos"]                     = [];
                            $lines['$_XREF_$']["infos"]["line"]             = $Counter;
                            $lines['$_XREF_$']["infos"]["start"]            = [];
